...
Before we proceed with more `Array` methods, I want to go back for a minute and talk about [the `.split` method from the `String` class](https://chapters-foo.ourplatform.com/chapters/757#split){:target="_blank"}. This method, when called on a `String`, will return an `Array` of substrings:
...
You can also negate `include?` with a `!` (prounounced "bang"), to find out whether a list _excludes_ a particular element:
...
Once we get to Ruby on Rails, a method called `exclude?` is added that does the same thing. If you want to use the `exclude?` method outside of Rails, then you must bring the activesupport library into your Ruby program.
